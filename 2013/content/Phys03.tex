...
摂動によって状態はこのように変化する。
\begin{equation}\begin{aligned}[b]
    \ket{8}'= \ket{8}-\sum_{m\neq n}\frac{\mel{m}{\hat{H}_{AH}}{8}}{E_m-E_8}\ket{m}
    =: \ket{8}+ \sum_{m=2}^{6}c_{2m}^{(1)}\ket{2m}
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}[b]
    ...
    &=q\sqrt{\frac{\hbar}{2m\omega_0}}(\hat{a}+\hat{a}^\dagger)\qty[
        \ket{8}- \sum_{m=2}^{6}c_{2m}^{(1)}\ket{2m}]\\
    &= q\sqrt{\frac{\hbar}{2m\omega_0}}\qty(\sqrt{8}\ket{7}+3\ket{9})+\sum_{m=2}^{7}c_{2m-1}^{(1)}\ket{2m-1}
\end{aligned}\end{equation}
となる。
