...
\end{aligned}\end{equation}
よって求める状態は2つあり、
1つは\(C_4=+i\)かつ\(b=ia\)の状態で、波動関数の規格化条件も考慮すると波動関数は
\begin{equation}\begin{aligned}[b]
    \Psi_{+i} = \frac{1}{\sqrt{2}}\qty(\Psi_{12}+i\Psi_{21})
\end{aligned}\end{equation}
である。
もう1つは\(C_4=-i\)かつ\(b=-i\)の状態で、波動関数の規格化条件も考慮すると波動関数は
\begin{equation}\begin{aligned}[b]
    \Psi_{-i} = \frac{1}{\sqrt{2}}\qty(\Psi_{12}-i\Psi_{21})
\end{aligned}\end{equation}
である。
...
なので\(\Psi_{+i}\)の状態の角運動量の期待値は
\begin{equation}\begin{aligned}[b]
    \ev{L} = +\hbar
\end{aligned}\end{equation}
\(\Psi_{-i}\)の状態の角運動量の期待値は
\begin{equation}\begin{aligned}[b]
    \ev{L} = -\hbar
\end{aligned}\end{equation}
...
実際波動関数を原点付近で展開すると
それぞれ\(3z^2-r^2,xy,(xz,yz)\)に比例するのがわかり、結晶場分裂で見覚えのある組になっている。
