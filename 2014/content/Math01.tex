...
\subsection*{解法2}
\(n\times n\)の行列の固有多項式は\(\lambda\)の\(n\)次の多項式となる。
ケーリーハミルトンの定理より、固有多項式の固有値を元の行列に置き換えても成り立つので、
\(A^k\)の無限和というのは\(A^{n-1}\)次までの行列の和で書くことができる。
今の場合は\(3\times3\)の行列なので
\begin{equation}\begin{aligned}[b]
    e^{tA} = a_0(t)I + a_1(t)A + a_2(t)A^2
\end{aligned}\end{equation}
のように書ける。
これまたケーリーハミルトンの定理より\(A\)を固有値\(\lambda\)に置き換えることで、
\(a(t)\)を求められる。
...
これより
\begin{equation}\begin{aligned}[b]
    a_1(t) &= \frac{e^{t}-e^{-t}}{2}, & a_2(t) &= 1+\frac{e^{t}+e^{-t}}{2}
\end{aligned}\end{equation}
とわかる。
